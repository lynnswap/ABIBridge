...
Use ``ABIRuntime`` to locate C, C++, or Swift symbols without writing mangled names. A ``ResolvedSymbol`` keeps its containing image alive and provides scoped access to the raw address. C++ callers can use typed C/C++ function handles through the `ABIBridgeCore` product. Swift typed invocation is not yet implemented.
